...
class LocationController:
    ...
    def add_city(city_name):
        ...
        return {'Sucessfully':f'{new_city.name} is SucessFully Added'}
    ...
    def delete_city(city_name):
        ...
        db.session.commit()
        return {'Successfully': f'{city.name} is successfully deleted'},200
...
#################################  Place ########################################################
    ...
    def delete_place(city_name,place_name):
        ...
        db.session.commit()
        return {'Successfully': f'{place_name} is successfully deleted'}, 200

    @staticmethod
    def update_place(city_name,place_name, new_name):
        # Fetch the Place by name
        city=db.session.query(City).filter(City.name==city_name).first()
        place = db.session.query(Place).filter(Place.name == place_name and Place.city_id == city.id).first()
